...
PKG_NAME="omniphony"
PKG_VERSION="5fee2a43dc8c09013e18499133dada0bf6639744"
PKG_SHA256="e4ec99d23cd40ab018db26be960a21c35c5b90a716740d14be9edec370c34632"
PKG_LICENSE="GPL-3.0-or-later"
PKG_SITE="https://github.com/mgth/Omniphony"
# The fork rather than PKG_SITE, and no longer for the tarball path alone: this
# is a fork commit upstream's main does not carry. It is upstream at 434f502
# plus one crate - pcm_bridge, the decoder bridge that presents host-decoded
# PCM as a channel bed, so the codec can render anything ffmpeg decodes rather
# than only the formats the object decoder handles - and two changes to the
# renderer: the resampled head model is built across cores, and the C ABI
# reports the rate the bridge actually decoded at.
#
# Both of those are here because the codec no longer opens the engine at one
# fixed rate. The measured set is stored at 48 kHz, so any other rate resamples
# 836 directions and reconstructs each ear's minimum phase before
# orender_create returns; on this hardware that was seconds, and the codec gave
# up waiting. Across cores it is about three times less.
#
# The other is what tells the codec which rate to open at in the first place.
# A host has to name one before it has seen a packet, and for DTS-HD MA the
# rate it can name is the core's - 48 kHz - while an XLL extension riding that
# core decodes at 96. orender_decoded_sample_rate answers for the last frame
# rendered, so the codec opens at its best guess and re-opens at what the
# engine says. It is additive, ABI minor 8, so the helper probes for it and an
# engine without it simply reports nothing.
#
# The fourth is orender_presentation_name, and it exists for the one format
# that never says what it is. An Auro-Codec carrier is an ordinary DTS-HD MA
# 5.1 in every field a demuxer reads: its height layer is folded into the low
# bits of the carrier's own PCM, so there is no syncword to test and no profile
# to read, and the track stays a plain 5.1 until a decoder has found the side
# channel and checked its CRC. By then the host has long since asked. So the
# bridge answers afterwards and the engine passes it up - "Auro 11.1", the
# layout a listener buys speakers for - and the helper puts it on the status
# line for the codec to show. Additive as well and probed the same way, empty
# for every stream whose name the host already had. It shares ABI minor 8 with
# the rate symbol rather than taking a 9: upstream's last is 7, so 8 is this
# fork's own counter, and these two are one set that ships together. The minor
# is a diagnostic either way - ABI.md gates features on symbol presence.
#
# So those four are what ride on the pin, and they are the only things that do.
# Everything else here is still upstream's. The LFE trim this image exposes
# uses a mechanism the renderer already has: a virtual-bed entry's per-channel
# gain is summed onto that channel's render gain, direct-routed or spatialized
# alike, so Kodi writes two bed rows rather than needing a key of its own. That
# gain is a float here, which the setting's half-decibel steps need.
#
...
# reflection being low-passed by its wall and its own air path. The distance
# default is 2 m for that reason, and the reverb 10%: set by ear on the device
# rather than carried over from when distance barely moved the level.
#
