...
PKG_NAME="omniphony"
PKG_VERSION="e83dc271f0b14fb0622e6f354c5355b225f821ae"
PKG_SHA256="eb403d370d232894580088b2d76e6c3b0f7f95aac983143e82b7f6d5df46bc18"
PKG_LICENSE="GPL-3.0-or-later"
PKG_SITE="https://github.com/mgth/Omniphony"
# The fork rather than PKG_SITE: upstream at 86d7f27 plus one crate and four
# renderer changes, one of which is upstream's own.
#
...
#     decoder handles.
#   - The bed labels are exported for a presentation that places any overhead
#     channel, not only for one carrying objects. Atmos hands over a sparse bed
#     the objects explain, but a DTS:X presentation can be a full floor and four
#     heights with no object at all, and gated on objects those ten placed
#     channels said nothing the host could tell from a plain 5.1 track.
#     orender_decoded_sample_rate ships with it, reporting the rate the bridge
#     actually decoded at: a host must name a rate before it has seen a packet,
...
#     and re-opens at what the engine says. So does
#     orender_presentation_name, which names the one format that never says
#     what it is - an Auro-Codec carrier is an ordinary DTS-HD MA 5.1 in every
#     field a demuxer reads, its height layer folded into the low bits of the
#     carrier's own PCM - so the bridge answers after the fact and the helper
#     puts "Auro 11.1" on the status line. Both are additive, ABI minor 8, and
#     probed by symbol presence, so an engine without them reports nothing.
#   - Auro-3D's speakers are placed where Auro puts them. The renderer grew the
#     AuroL..AuroT labels and their angles, which is what lets the bridge's own
#     Auro commit say which channel is which; the shared room-corner labels it
#     used to borrow put the height surrounds behind the listener.
#   - The resampling kernel is tabulated and the FFT plans reused. This one is
#     upstream's - their PR #543, which replaced our own across-cores commit
#     with a better version of the same fix and kept the parallel pass - so it
#     is cherry-picked rather than carried, and ours is dropped. Building the
#     measured head model at a non-native rate took most of a second before
#     orender_create returned, which the codec gave up waiting for.
#   - orender_drain renders what the decoder is still holding when the input
#     ends. A bridge that buffers an access unit to see what follows it has one
#     in hand at end of stream and nothing left to release it, so the tail of
#     every track was dropped - 32 ms for E-AC-3. Additive and probed like the
#     rest, and in the same ABI minor 8: a host that never calls it behaves
#     exactly as before. The harletty pin is what implements it.
#
# Upstream's last minor is 7, so 8 is this fork's whole set over it rather than
# a number per symbol - the minor moves when upstream's does, and ABI.md gates
# features on symbol presence rather than on the number. The helper already
# probes every optional symbol with dlsym, so it reads an engine with the drain
# and one without identically; what it does not yet do is call it, so the tail
# this releases does not reach Kodi until FLUSH is wired to it.
#
# Everything else is upstream's. The LFE trim uses a mechanism the renderer
# already has - a virtual-bed entry's per-channel gain is summed onto that
# channel's render gain - so Kodi writes two bed rows rather than needing a key
# of its own. Reflections and the reverb send scale relative to the direct sound
# instead of by absolute laws that saturated a metre out, so both track
# unit_scale_m, which the speaker-distance setting drives; the 2 m distance and
# 10 % reverb defaults were set by ear on the device rather than carried over
# from when distance barely moved the level. The embedded KEMAR grid is 5
# degrees rather than 10, about 8 MB against 2 - worth knowing on this box,
# since upstream sized it for a PC. Diffuse-field equalisation is opt-in and
# this image asks for it; the codec's WriteConfig says why. The C ABI this
# ships is 0.8 - major 0, and the fork's minor described above.
...
#
# A fifth is installed ahead of the codec that names it: libpcm_bridge.so, which
# is only reached once the codec can feed the renderer decoded PCM. Shipping it
# with the engine keeps the image and the engine pin one decision rather than
# two, and an unused .so costs the image 0.5 MB stripped and nothing at runtime.
