 ...
 *
 * FLUSH acknowledges and does nothing else: it cannot extract audio the
 * decoder is still holding. It exists so the host has a defined way to mark
 * the end of a segment now, rather than the protocol gaining an incompatible
 * sixth command later.
 *
 * That was once a limit of the ABI - `orender_process` was the only way to get
 * audio out and there was no drain call - and is now a limit of this helper.
 * The engine pinned here exposes `orender_drain`, which renders exactly what
 * this comment says FLUSH cannot reach: an E-AC-3 independent access unit is
 * held back until the next one says whether a dependent follows it, so one is
 * always held when a track ends, and it is dropped rather than played - the
 * last 32 ms of every E-AC-3 track. Wiring it is a `dlsym` beside the others
 * and a call in OP_FLUSH emitting its audio the way OP_FEED does; FLUSH is
 * still where it goes. Until then the engine can release that tail and nothing
 * asks it to.
 *
 *
 * ON DEADLOCK
...
typedef struct
{
  ...
  uint32_t (*presentation_name)(const OrenderRenderer*, uint8_t*, uint32_t);
...
#undef SYM
  ...
  *(void**)(&api.presentation_name) = dlsym(lib_handle, "orender_presentation_name");
 ...
 * "Auro 11.1" - and empty for everything the host already has a name for.
 *
...
static void describe_presentation(OrenderRenderer* r, char* out, size_t cap)
{
  out[0] = '\0';
  if (!api.presentation_name)
    return;

  /* Query, then fill, exactly as describe_bed does: the ABI writes nothing
   * unless cap >= N, so a name longer than this buffer yields an empty
   * description rather than a truncated one. No terminator is written, so the
   * cap check leaves room for the one added here. */
  const uint32_t n = api.presentation_name(r, NULL, 0);
  if (n == 0 || n >= cap || api.presentation_name(r, (uint8_t*)out, n) != n)
    return;
  out[n] = '\0';
...
int main(void)
{
  ...
  uint32_t last_rate = UINT32_MAX; /* 0 is "the engine has not decoded yet" */
  /* Compared by value rather than by a count: the name is what changes, and a
   * carrier can be ruled out or confirmed without any other field moving. */
  char last_presentation[64] = "";
  ...
  for (;;)
  {
    ...
    switch (op)
    {
      ...
      case OP_FEED:
      {
        ...
        /* What the bridge actually decoded at, which is not always what this
         * helper was told to build the engine for: the host has to name a rate
         * before a packet exists, and for DTS XLL - a 96 kHz extension carried
         * over a 48 kHz core - the core sync word it read says 48. The host
         * cannot discover that on its own; nothing downstream of it can either,
         * because a whole film at half speed is not an error anywhere. So it is
         * reported here, and the host re-opens if it disagrees. 0 from an engine
         * that has not decoded a frame yet, and from one too old to have the
         * symbol - both mean "no answer", which is what the host must not act
         * on. */
        const uint32_t rate = api.decoded_sample_rate ? api.decoded_sample_rate(renderer) : 0;

        char presentation[64];
        describe_presentation(renderer, presentation, sizeof(presentation));

        if (!reported_info || objects != last_objects || spatial != last_spatial ||
            bed_n != last_bed_n || rate != last_rate ||
            strcmp(presentation, last_presentation) != 0)
        {
          /* bed stays last, and never contains a space, so a host can read it to
           * the end of the line without the key order mattering. Anything new
           * goes before it. */
          char bed[128];
          describe_bed(renderer, bed, sizeof(bed));
          emit_status(ST_INFO,
                      "stream objects=%d spatial=%d channels=%u rate=%u presentation=%s bed=%s",
                      objects, spatial, channels, rate, presentation, bed);
          reported_info = 1;
          last_objects = objects;
          last_spatial = spatial;
          last_bed_n = bed_n;
          last_rate = rate;
          snprintf(last_presentation, sizeof(last_presentation), "%s", presentation);
        }
      ...
      case OP_RESET:
        ...
        last_rate = UINT32_MAX;
        last_presentation[0] = '\0';
